//  Bismillah ar-Rahmaan ar-Raheem  Easylogging++ v9.89
//  Single-header only, cross-platform logging library for C++ applications
//  Copyright (c) 2017 muflihun.com   This library is released under the MIT Licence.
//  http://easylogging.muflihun.com/licence.php          https://github.com/easylogging/easyloggingpp   http://easylogging.muflihun.com  http://muflihun.com
//  2024 年11月  刘文庆修改使用c++builder XE 12版本 
//  2024 年11月  刘文庆修改在编译器支持C++11版本情况下，利用inline 变量定义，避免必须使用INITIALIZE_EASYLOGGINGPP初始化
//  注意：easylogging++日志库，如果使用ELPP_THREAD_SAFE宏，需在int argc, char *argv[]函数后面跟随START_EASYLOGGINGPP(argc, argv);宏定义，
//        该宏定义没有Unicode宽字符集的实现定义，为了使得日志库能够在winmain()函数中使用，我们设计了如下代码：
//         #include <shellapi.h>
//       void  SetEasyLoggingSafeThread()  //该函数仅仅限制在winmain()函数中使用，故不包含在头文件中
//       {
//          int argc = 0;
//
...
//2024 年11月30日  刘文庆修改强制采用宽字符集和线程安全的代码
//使用Unicode字符集
#ifndef ELPP_UNICODE
#define ELPP_UNICODE
#endif
...
#ifndef __DEFINE_EASYLOGGINGPP__
#define __DEFINE_EASYLOGGINGPP__
#ifndef __HAS_CPP17__
#	ifdef _MSVC_LANG
#		if _MSVC_LANG > 201402L
#			define __HAS_CPP17__	1
...
namespace el
{
    namespace base
    {
        namespace utils
        {
            inline   std::string s_currentUser = el::base::utils::OS::currentUser();
            inline    std::string s_currentHost = el::base::utils::OS::currentHost();
            inline   bool s_termSupportsColor = el::base::utils::OS::termSupportsColor();
        }
        inline  el::base::type::StoragePointer elStorage(new el::base::Storage(el::LogBuilderPtr(new el::base::DefaultLogBuilder())));
    }
    inline  el::base::debug::CrashHandler elCrashHandler(ELPP_USE_DEF_CRASH_HANDLER);
}
#else
   //请在前main()使用 INITIALIZE_EASYLOGGINGPP
#endif
